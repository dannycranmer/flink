...
public class ShardConsumer<T> implements Runnable {
	...
	public ShardConsumer(KinesisDataFetcher<T> fetcherRef,
		...
		if (lastSequenceNum.equals(SENTINEL_AT_TIMESTAMP_SEQUENCE_NUM.get())) {
			...
			startingPosition = StartingPosition.fromSequenceNumber(checkNotNull(lastSequenceNum));
		}
	}
	...
	public void run() {
		try {
			recordPublisher.subscribe(startingPosition, batch -> {
			...
			while (isRunning()) {
				...
				recordPublisher.run(lastSequenceNum);
			}
		} catch (Throwable t) {
	 ...
	 */
	private void deserializeRecordForCollectionAndUpdateState(@Nonnull final UserRecord record) {
	...
	private boolean filterDeaggregatedRecord(@Nonnull final UserRecord record) {
