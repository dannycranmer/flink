...
public class StartingPosition {
	...
	private StartingPosition(@Nonnull final ShardIteratorType shardIteratorType, @Nullable final Object startingMarker) {
	...
	@Nonnull
	public static StartingPosition fromTimestamp(@Nonnull final Date date) {
		return new StartingPosition(AT_TIMESTAMP, date);
	}

	@Nonnull
	public static StartingPosition fromSequenceNumber(@Nonnull final SequenceNumber sequenceNumber) {
		if (isSentinelSequenceNumber(sequenceNumber)) {
			...
			return new StartingPosition(getShardIteratorType(sequenceNumber), sequenceNumber.getSequenceNumber());
		}
	}

	@Nonnull
	private static ShardIteratorType getShardIteratorType(@Nonnull final SequenceNumber sequenceNumber) {
		return sequenceNumber.isAggregated() ? AT_SEQUENCE_NUMBER : AFTER_SEQUENCE_NUMBER;
	}

	@Nonnull
	private static ShardIteratorType fromSentinelSequenceNumber(@Nonnull final SequenceNumber sequenceNumber) {
